...
/**
 * Whatsapptenantstemplate model.
 *
 * @since  1.0.0
 */
class WhatsapptenantstemplateModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_DT_WHATSAPP_TENANTS_TEMPLATES';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_dt_whatsapp_tenants_templates.whatsapptenantstemplate';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
								'com_dt_whatsapp_tenants_templates.whatsapptenantstemplate', 
								'whatsapptenantstemplate',
								array(
									'control' => 'jform',
									'load_data' => $loadData 
								)
	...
	/**
	 * Method to duplicate an Whatsapptenantstemplate
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_dt_whatsapp_tenants_templates'))
		{
		...
		foreach ($pks as $pk)
		{
			if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser())
			{
				...
				if (!empty($table->header_media))
				{
					if (is_array($table->header_media))
					{
						$table->header_media = implode(',', $table->header_media);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__dt_whatsapp_tenants_templates');
				$max             = $db->loadResult();
