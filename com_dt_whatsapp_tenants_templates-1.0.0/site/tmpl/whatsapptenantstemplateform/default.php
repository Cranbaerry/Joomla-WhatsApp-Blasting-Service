...
<div class="whatsapptenantstemplate-edit front-end-edit">
	...
	<?php else: ?>
		...
		<form id="form-whatsapptenantstemplate"
			...
			<?php if (!empty($this->item->header_media)): ?>
				...
				<?php foreach ((array) $this->item->header_media as $fileSingle): ?>
					<?php if (!is_array($fileSingle)): ?>
						<a
							href="<?php echo Route::_(Uri::root() . 'uploads' . DIRECTORY_SEPARATOR . Factory::getApplication()->getIdentity()->id . DIRECTORY_SEPARATOR . $fileSingle, false); ?>"><?php echo $fileSingle; ?></a>
						|
						<?php $header_mediaFiles[] = $fileSingle; ?>
					<?php endif; ?>
				<?php endforeach; ?>
				<input type="hidden" name="jform[header_media_hidden]" id="jform_header_media_hidden"
					value="<?php echo implode(',', $header_mediaFiles); ?>" />
			<?php endif; ?>
			...
			<input type="hidden" name="task" value="whatsapptenantstemplateform.save" />
