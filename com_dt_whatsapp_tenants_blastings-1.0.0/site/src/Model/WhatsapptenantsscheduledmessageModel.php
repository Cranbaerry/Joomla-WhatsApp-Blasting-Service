 ...
 * @since  1.0.0
 */
class WhatsapptenantsscheduledmessageModel extends ItemModel
{
	public $_item;
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_dt_whatsapp_tenants_blastings')) && (!$user->authorise('core.edit', 'com_dt_whatsapp_tenants_blastings')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_dt_whatsapp_tenants_blastings.edit.whatsapptenantsscheduledmessage.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_dt_whatsapp_tenants_blastings.edit.whatsapptenantsscheduledmessage.id', $id);
		}

		$this->setState('whatsapptenantsscheduledmessage.id', $id);
		...
		{
			$this->setState('whatsapptenantsscheduledmessage.id', $params_array['item_id']);
		}
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if (empty($id))
			{
				$id = $this->getState('whatsapptenantsscheduledmessage.id');
			}
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_DT_WHATSAPP_TENANTS_BLASTINGS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_DT_WHATSAPP_TENANTS_BLASTINGS_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->template_id) && $this->_item->template_id != '')
		{
			...
			{
				$this->_item->template_id = ArrayHelper::fromObject($this->_item->template_id);
			}

			$values = (is_array($this->_item->template_id)) ? $this->_item->template_id : explode(',',$this->_item->template_id);
	 ...
	 */
	public function getTable($type = 'Whatsapptenantsscheduledmessage', $prefix = 'Administrator', $config = array())
	{
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('whatsapptenantsscheduledmessage');
		}
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('whatsapptenantsscheduledmessage.id');
