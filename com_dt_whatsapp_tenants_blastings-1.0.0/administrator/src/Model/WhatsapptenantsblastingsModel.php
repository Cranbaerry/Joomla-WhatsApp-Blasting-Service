...
/**
 * Methods supporting a list of Whatsapptenantsblastings records.
 *
 * @since  1.0.0
 */
class WhatsapptenantsblastingsModel extends ListModel
{
	...
	*/
	...
	protected function getListQuery()
	{
		...
		if ($filter_mode !== null && (is_numeric($filter_mode) || !empty($filter_mode)))
		{
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->template_id))
			{
				...
				$oneItem->template_id = !empty($textValue) ? implode(', ', $textValue) : $oneItem->template_id;
					...
					$oneItem->mode = !empty($oneItem->mode) ? Text::_('COM_DT_WHATSAPP_TENANTS_BLASTINGS_WHATSAPPTENANTSBLASTINGS_MODE_OPTION_' . preg_replace('/[^A-Za-z0-9\_-]/', '',strtoupper(str_replace(' ', '_',$oneItem->mode)))) : '';
		}
