...
/**
 * Whatsapptenantsscheduledmessage model.
 *
 * @since  1.0.0
 */
class WhatsapptenantsscheduledmessageModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_DT_WHATSAPP_TENANTS_BLASTINGS';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_dt_whatsapp_tenants_blastings.whatsapptenantsscheduledmessage';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
								'com_dt_whatsapp_tenants_blastings.whatsapptenantsscheduledmessage', 
								'whatsapptenantsscheduledmessage',
								array(
									'control' => 'jform',
									'load_data' => $loadData 
								)
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_dt_whatsapp_tenants_blastings.edit.whatsapptenantsscheduledmessage.data', array());
	...
	/**
	 * Method to duplicate an Whatsapptenantsscheduledmessage
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_dt_whatsapp_tenants_blastings'))
		{
		...
		foreach ($pks as $pk)
		{
			if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser())
			{
				...
				if (!empty($table->template_id))
				{
					...
					{
						$table->template_id = implode(',', $table->template_id);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__dt_whatsapp_tenants_scheduled_messages');
				$max             = $db->loadResult();
